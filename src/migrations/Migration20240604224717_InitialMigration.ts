...
export class Migration20240604224717_InitialMigration extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "purchase_history" ("id" uuid not null, "ingredient_id" uuid not null, "quantity_purchased" int not null, "purchased_at" timestamptz not null, constraint "purchase_history_pkey" primary key ("id"));');
  ...
  async down(): Promise<void> {
    this.addSql('alter table "purchase_history" drop constraint "purchase_history_ingredient_id_foreign";');
